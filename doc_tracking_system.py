...
class DocTrackingSystem:
    ...
    def list_documents(self):
        ...
        cursor.execute('''
            SELECT d.id, d.doc_number, d.doc_title, d.project_number, d.status,
                   s.shipment_date, s.recipient, r.return_date
            FROM documents d
            LEFT JOIN shipments s ON d.id = s.doc_id
            LEFT JOIN returns r ON d.id = r.doc_id
            ORDER BY d.id
        ''')
