...
class UploadedFile(Base):
    ...
    original_filename = Column(String, nullable=False)     # Оригинальное имя файла от пользователя
    ...
    category_id = Column(Integer, ForeignKey("file_categories.id"), nullable=True)  # Категория файла
    section_id = Column(String, nullable=True)             # ID секции, с которой связан файл
    section_name = Column(String, nullable=True)           # Название секции
    project_id = Column(String, nullable=True)             # ID проекта
    uploaded_by = Column(String, nullable=False)           # Кто загрузил файл
    description = Column(Text, nullable=True)              # Описание файла
    is_active = Column(Boolean, default=True)              # Активен ли файл
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
...
class MaterialRequest(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    material_id = Column(Integer, ForeignKey("materials.id"), nullable=False)  # ID материала
    ...
    section_id = Column(String, nullable=True)                # ID секции, где требуется материал
    section_name = Column(String, nullable=True)              # Название секции
    project_id = Column(String, nullable=True)                # ID проекта
    reason = Column(Text, nullable=True)                      # Причина запроса
    status = Column(String, default="pending")                # Статус запроса: pending, approved, rejected, fulfilled
    requested_by = Column(String, nullable=False)             # Кто запросил
    approved_by = Column(String, nullable=True)               # Кто одобрил
...
class MaterialStock(Base):
    ...
    location = Column(String, nullable=True)                  # Местоположение склада
